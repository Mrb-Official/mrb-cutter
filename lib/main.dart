Yeh lo pura updated `main.dart` — aspect ratio feature ke saath:
...
class _HomeScreenState extends State<HomeScreen> {
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Wrap(
                spacing: 8,
                children: ['9:16', '1:1', '16:9'].map((ratio) {
                  return ChoiceChip(
                    label: Text(ratio),
                    selected: selected == ratio,
                    onSelected: (_) =>
                        setDialogState(() => selected = ratio),
                  );
                }).toList(),
              ),
              const SizedBox(height: 20),
              const Text("Custom Ratio",
                  style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 10),
              Row(
                children: [
            ...
            FilledButton(
              onPressed: () {
                if (customWidth.isNotEmpty && customHeight.isNotEmpty) {
                  setState(() => _aspectRatio = '$customWidth:$customHeight');
                } else {
                  setState(() => _aspectRatio = selected);
                }
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text("Aspect Ratio set: $_aspectRatio")),
                );
              },
  ...
  Future<void> processMovie() async {
    try {
      ...
      String outputDirPath =
          '/storage/emulated/0/Download/MRB_$movieName';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('MRB DASHBOARD',
            style: TextStyle(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                          "Welcome, Creator. Ratio: $_aspectRatio | Select a movie to generate high-quality reels.",
            ...
            if (_isProcessing)
              ...
              FilledButton.icon(
                ...
                label: const Text("Process & Generate Reels",
                    style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                        letterSpacing: 1)),
...
GitHub pe `lib/main.dart` replace karo — build karo. Settin            fontFamily: 'Roboto',
          ),
          home: const HomeScreen(),
        );
      },
